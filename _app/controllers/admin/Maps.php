...
class Maps extends Backend_Controller {
	...
	function show() {
		...
		$this->template->content( "admin/view_maps", $data );
		$this->template->show( THEMES_BACKEND . 'index' );
	}

	function latlng_indo()
	{
		$area = $this->input->post('area', true);
		$wilayah = $this->dashboard_model->get_wilayah($area);

		$arr_pin = [];
		foreach ($wilayah as $key => $v) {
			$ln = $this->dashboard_model->get_latlng($v['wilayah'], $area);
			$arr_pin[] = [
				'wilayah' => $v['wilayah'],
				'total' => $v['total'],
				'lat' => $ln['lat'],
