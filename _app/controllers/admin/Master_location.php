...
class Master_location extends Backend_Controller {
	...
	function show() {
		...
		$data['title'] = 'Master Locations';
		$this->template->title( $data['title'] );
		$this->template->content( "admin/master_location", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$input['s_kprov'] != '' ? $where .= "bps_province_code LIKE '%" . $input['s_kprov'] . "%' AND " : '';
			$input['s_kkab'] != '' ? $where .= "bps_regency_code LIKE '%" . $input['s_kkab'] . "%' AND " : '';
			$input['s_kkec'] != '' ? $where .= "bps_district_code LIKE '%" . $input['s_kkec'] . "%' AND " : '';
			$input['s_kkel'] != '' ? $where .= "bps_village_code LIKE '%" . $input['s_kkel'] . "%' AND " : '';
			$input['s_loc'] != '' ? $where .= "location_id LIKE '%" . $input['s_loc'] . "%' AND " : '';
			$input['s_bps'] != '' ? $where .= "bps_full_code LIKE '%" . $input['s_bps'] . "%' AND " : '';
			...
			WHERE $where 1=1
			ORDER BY location_id OFFSET $start ROWS
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_locations WHERE $where 1=1";
