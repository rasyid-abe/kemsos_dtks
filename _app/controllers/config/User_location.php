...
class User_Location extends Backend_Controller {
    ...
    function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$input['s_user_id'] != '' ? $where .= "user_id = '" . $input['s_user_id'] . "' AND " : '';
			$input['s_grups'] != '' ? $where .= "group_name LIKE '%" . $input['s_grups'] . "%' AND " : '';
			$input['s_namas'] != '' ? $where .= "nama_depan LIKE '%" . $input['s_namas'] . "%' AND " : '';
			$input['s_user'] != '' ? $where .= "username LIKE '%" . $input['s_user'] . "%' AND " : '';
			...
			$sql = "
				SELECT * FROM vw_user_location
				WHERE $where 1=1
				ORDER BY user_id DESC OFFSET $start ROWS
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_user_location WHERE $where 1=1";
