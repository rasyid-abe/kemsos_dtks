...
class Per_enumerator_detail extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->library('excel');
		$this->dir = base_url( 'dashboard/per_enumerator_detail/' );
	...
	function show() {
		...
		$data['title'] = 'Detail Transaksi Per Enumerator';
		$this->template->title( $data['title'] );
		$this->template->content( "admin/rekap/detail_per_enum", $data );
	...
	function get_show_data() {
		...
		if (!in_array('p-i-c', $is_pic) || $input['s_fill'] != 0) {
			...
			// $input['s_krt'] != '' ? $where .= "krt_name LIKE '%" . $input['s_krt'] . "%' AND " : '';
			// $input['s_art'] != '' ? $where .= "art_name LIKE '%" . $input['s_art'] . "%' AND " : '';
			$phone = $enum = $krt = $art = '';
			$input['s_ph'] != '' ? $phone = $input['s_ph'] : '';
			$input['s_enum'] != '' ? $enum = $input['s_enum'] : '';
			$input['s_krt'] != '' ? $krt = $input['s_krt'] : '';
			$input['s_art'] != '' ? $art = $input['s_art'] : '';
			...
			$sql = "
			EXEC dbo.sp_Detail_Transaksi_Per_Enum '". $region."', '". $krt."', '". $art."', '". $enum."', '". $phone."', '".$start."', '".$length."', NULL
			";
			...
			foreach($gen_data->result() as $rows)
			{
				...
				$rows->nama_art,
				$status_art,
				$rows->status_padan > 0 ? 'Ya' : 'Tidak',
				$rows->nama_enum,
				$rows->username,
				$status
				);
			}
			$sql_total = "EXEC dbo.sp_Detail_Transaksi_Per_Enum '". $region."', '". $krt."', '". $art."', '". $enum."', '". $phone."', '1', '1', '1'";
	...
	public function export_data(){
		$data = array();
		$where = $region = '';
		$s_prov = $this->input->post('prov');
		$s_regi = $this->input->post('regi');
		$s_dist = $this->input->post('dist');
		$s_vill = $this->input->post('vill');
		...
		$excel->setActiveSheetIndex(0)->setCellValue('E1', "id_prelist");
		$excel->setActiveSheetIndex(0)->setCellValue('F1', "nama_krt");
		$excel->setActiveSheetIndex(0)->setCellValue('G1', "nama_art");
		$excel->setActiveSheetIndex(0)->setCellValue('H1', "keberadaan_art");
		$excel->setActiveSheetIndex(0)->setCellValue('I1', "status_padan");
		$excel->setActiveSheetIndex(0)->setCellValue('J1', "nama_enum");
		$excel->setActiveSheetIndex(0)->setCellValue('K1', "username");
		$excel->setActiveSheetIndex(0)->setCellValue('L1', "stereotype");
		...
		$sql_total = "EXEC dbo.sp_Detail_Transaksi_Per_Enum '". $region."', '', '', '', '', '1', '1', '1'";
        $total_data = $this->totalData($sql_total);

		$sql = "
			EXEC dbo.sp_Detail_Transaksi_Per_Enum '". $region."', '', '', '', '', '1', '".$total_data."', NULL
		";
        ...
        {
			if($value['keberadaan_art'] == 1) {
				$status_art = "Terkonfirmasi";
			} else if($value['keberadaan_art'] == 2) {
				$status_art = "Tidak Terkonfirmasi";
			} else if($value['keberadaan_art'] == 3) {
				$status_art = "Ganda";
			} else if($value['keberadaan_art'] == 4) {
				$status_art = "Meninggal";
			} else if($value['keberadaan_art'] == 5) {
				$status_art = "Tidak Memiliki/Belum tercatat dalam dokumen kependudukan
				";
			}
			...
			$excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $value['kabupaten'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $value['kecamatan'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $value['kelurahan'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $value['id_prelist'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $value['nama_krt'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $value['nama_art'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, $status_art,PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('I'.$numrow, $value['status_padan'] > 0 ? 'Ya' : 'Tidak',PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('J'.$numrow, $value['enum_name'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('K'.$numrow, $value['username'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('L'.$numrow, $value['stereotype'],PHPExcel_Cell_DataType::TYPE_STRING);
