...
class Per_wilayah_art extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->library('excel');
		$this->dir = base_url( 'dashboard/per_wilayah_art/' );
	...
	function show() {
		...
		$data['title'] = 'Rekapitulasi Per Wilayah (ART)';
		$this->template->title( $data['title'] );
		$this->template->content( "admin/rekap/per_wilayah_art", $data );
	...
	function get_show_data_art() {
		...
		if (!in_array('p-i-c', $is_pic) || $input['s_fill'] != 0) {
			...
			$sql = "
			EXEC dbo.stp_rekap_wilayah_art '". $region."', '".$start."', '".$length."'
			";
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_rekap_per_wilayah_art WHERE $where 1=1 $where_in";
	...
	public function export_data(){
		...
		$excel->setActiveSheetIndex(0)->setCellValue('A1', "bps_full_code");
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "propinsi");
		$excel->setActiveSheetIndex(0)->setCellValue('C1', "kabupaten");
		$excel->setActiveSheetIndex(0)->setCellValue('D1', "kecamatan");
		$excel->setActiveSheetIndex(0)->setCellValue('E1', "kelurahan");
		$excel->setActiveSheetIndex(0)->setCellValue('F1', "terkonfirmasi");
		$excel->setActiveSheetIndex(0)->setCellValue('G1', "tidak_terkonfirmasi");
		$excel->setActiveSheetIndex(0)->setCellValue('H1', "ganda");
		$excel->setActiveSheetIndex(0)->setCellValue('I1', "meninggal");
		$excel->setActiveSheetIndex(0)->setCellValue('J1', "no_doc");
		$excel->setActiveSheetIndex(0)->setCellValue('K1', "total_selesai");
		$excel->setActiveSheetIndex(0)->setCellValue('L1', "target_art");
		...
		$sql = "
			EXEC dbo.export_rekap_wilayah_art '". $region."'
		";
		...
		foreach ($data->result_array() as $key => $value)
        {
			...
			$excel->setActiveSheetIndex(0)->setCellValue('A'.$numrow, $value['bps_full_code'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$numrow, $value['propinsi'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $value['kabupaten'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$numrow, $value['kecamatan'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$numrow, $value['kelurahan'],PHPExcel_Cell_DataType::TYPE_STRING);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$numrow, $value['terkonfirmasi']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$numrow, $value['tidak_terkonfirmasi']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('H'.$numrow, $value['ganda']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('I'.$numrow, $value['meninggal']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('J'.$numrow, $value['no_doc']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('K'.$numrow, $value['total_selesai']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('L'.$numrow, $value['target_art']);
		...
		header('Content-Disposition: attachment; filename="rekap_per_wilayah (art)"'.date('Y-m-d').'".xlsx"'); // Set nama file excel nya
