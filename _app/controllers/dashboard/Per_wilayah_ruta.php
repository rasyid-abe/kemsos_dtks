...
class Per_wilayah_ruta extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->library('excel');
		$this->dir = base_url( 'dashboard/Per_wilayah_ruta/' );
	...
	function show() {
		...
		$data['title'] = 'Rekapitulasi Per Wilayah (RUTA)';
		$this->template->title( $data['title'] );
		$this->template->content( "admin/rekap/per_wilayah_ruta", $data );
		$this->template->show( THEMES_BACKEND . 'index' );
	}

	function get_show_data_ruta() {
		$is_pic = $this->user_info['user_group'];
		...
		if (!in_array('p-i-c', $is_pic) || $input['s_fill'] != 0) {
			...
			$sql = "
			EXEC dbo.stp_rekap_wilayah_ruta '". $region."', '".$start."', '".$length."'
			";
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_rekap_per_wilayah_ruta WHERE $where 1=1 $where_in";
	...
	public function export_data(){
		...
		$excel->setActiveSheetIndex(0)->setCellValue('F1', "status_m1");
		$excel->setActiveSheetIndex(0)->setCellValue('G1', "status_m2");
		$excel->setActiveSheetIndex(0)->setCellValue('H1', "status_m3");
		$excel->setActiveSheetIndex(0)->setCellValue('I1', "total");
		...
		$sql = "
			EXEC dbo.export_rekap_wilayah_ruta '". $region."'
		";
		...
		foreach ($data->result_array() as $key => $value)
        {
			...
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$numrow, $value['status_m1']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$numrow, $value['status_m2']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('H'.$numrow, $value['status_m3']);
			$excel->setActiveSheetIndex(0)->setCellValueExplicit('I'.$numrow, $value['total']);
		...
		header('Content-Disposition: attachment; filename="rekap_per_wilayah (ruta)"'.date('Y-m-d').'".xlsx"'); // Set nama file excel nya
