...
class Qc_ruta extends Backend_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'disdukcapil/qc_ruta/' );
	...
	function show()	{
		...
		$data['title'] = 'Daftar Rumah Tangga (C1 dan C1a)';
		$this->template->title( $data['title'] );
		$this->template->content( "disdukcapil/view_qc_ruta", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_semua_data_c1 WHERE $where 1=1 $where_in";
