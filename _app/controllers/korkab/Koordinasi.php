...
class Koordinasi extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'korkab/koordinasi/' );
	...
	function show()	{
		...
		$data['cari2'] = $this->form_cari2();
		$data['base_url'] = $this->dir;
		$data['title'] = 'Daftar Foto Koordinasi';
		$this->template->title( $data['title'] );
		$this->template->content( "korkab/view_koordinasi", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			$input['s_usr'] != '' ? $where .= "username LIKE '%" . $input['s_usr'] . "%' AND " : '';
			$input['s_jns'] != '' ? $where .= "stereotype = '" . $input['s_jns'] . "' AND " : '';
			...
			$sql = "
				EXEC dbo.stp_foto_koordinasi '". $region."', '".$input['s_usr']."', '".$input['s_jns']."', '".$start."', '".$length."'
			";
			...
			{
				$by = $rows->username . ' | ' .$rows->nama_depan;
				$url = 'https://api-mkdtks.kemensos.go.id/';
				$image = '<a href="'.$url.substr($rows->internal_filename, 2).'" target="_blank">
				<img src="'.$url.substr($rows->internal_filename, 2).'" style="height:150px;width:150px">
				</a>';
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_foto_kegiatan WHERE $where 1=1";
	...
	function get_show_dataK() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			$input['s_kepdes'] != '' ? $where .= "kontak_kepala_desa LIKE '%" . $input['s_kepdes'] . "%' AND " : '';
			$input['s_operator'] != '' ? $where .= "kontak_operator_desa LIKE '%" . $input['s_operator'] . "%' AND " : '';
			...
			$valid_columns = array(
				...
				4=>'district_name',
				5=>'village_name',
				6=>'kontak_kepala_desa',
				7=>'kontak_operator_desa',
				8=>'created_on'
			);
			...
			$sql = "
				SELECT *
				FROM koordinasi kor
				LEFT JOIN ref_locations loc ON kor.bps_code = loc.bps_full_code
				WHERE $where 1=1
				ORDER BY kor.created_on DESC OFFSET $start ROWS FETCH NEXT $length ROWS ONLY
			";
			...
			foreach($gen_data->result() as $rows)
			{
				$on = date("d-m-Y H:i:s",strtotime($rows->created_on));
				$detail = '<a 	href="javascript:;"
								data-bps_full_code="'. $rows->bps_full_code .'"
								data-jml_art_terkonfirmasi="'. $rows->jml_art_terkonfirmasi .'"
								data-jml_art_tidak_terkonfirmasi="'. $rows->jml_art_tidak_terkonfirmasi .'"
								data-jml_art_meninggal="'. $rows->jml_art_meninggal .'"
								data-jml_art_ganda="'. $rows->jml_art_ganda .'"
								data-jml_art_no_dokumen="'. $rows->jml_art_no_dokumen .'"
								data-total_art_hasil_perbaikan="'. $rows->total_art_hasil_perbaikan .'"
								data-latitude="'. $rows->latitude .'"
								data-longitude="'. $rows->longitude .'"
								data-lastupdate_on="'. $rows->lastupdate_on .'"
								data-toggle="modal"
								data-target="#detail-koor"
				...
				$data[]= array(
					...
					$rows->district_name,
					$rows->village_name,
					$rows->kontak_kepala_desa,
					$rows->kontak_operator_desa,
					$on ." WIB"
				);
			}
			$sql_totalk = "SELECT COUNT(*) as num FROM koordinasi kor LEFT JOIN ref_locations loc ON kor.bps_code = loc.bps_full_code  WHERE $where 1=1";
			$total_data = $this->totalDataK($sql_totalk);
			$output = array(
				"draw" => $draw,
				"recordsTotal" => $total_data,
				"recordsFiltered" => $total_data,
	...
	function totalDataK($sql_totalk) {
		$query = $this->db->query($sql_totalk);
		$result = $query->row();
		if(isset($result)) return $result->num;
		return 0;
	}
	...
	function form_cari2() {
		$user_location = $this->get_user_location2();
		$jml_negara = count( explode( ',', $user_location['country_id'] ) );
		$jml_propinsi = count( explode( ',', $user_location['province_id'] ) );
		$jml_kota = count( explode( ',', $user_location['regency_id'] ) );
		$jml_kecamatan = count( explode( ',', $user_location['district_id'] ) );
		...
		<div class="row"">
		<div class="form-group col-md-3">
		<select id="select-propinsi2" style="width:100%" name="propinsi" class="select2 form-control" ' . ( ( ( $jml_propinsi == '1') && ( ! empty( $user_location['province_id'] ) ) ) ? 'disabled ' : '' ) . '>
		' . $option_propinsi . '
		</select>
		</div>
		<div class="form-group col-md-3">
		<select id="select-kabupaten2" style="width:100%" name="kabupaten" class="select2 form-control" ' . ( ( ( $jml_kota == '1' ) && ( ! empty( $user_location['regency_id'] ) ) ) ? 'disabled ' : '' ) . '>
		' . $option_kota . '
		</select>
		</div>
		<div class="form-group col-md-3">
		<select id="select-kecamatan2" style="width:100%" name="kecamatan" class="select2 form-control" ' . ( ( ( $jml_kecamatan == '1' ) && ( ! empty( $user_location['district_id'] ) ) ) ? 'disabled ' : '' ) . '>
		' . $option_kecamatan . '
		</select>
		</div>
		<div class="form-group col-md-3">
		<select id="select-kelurahan2" style="width:100%" name="kelurahan" class="select2 form-control" >
		' . $option_kelurahan . '
		</select>
		</div>
		</div>
