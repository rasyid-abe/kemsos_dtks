...
class Publish_data_p2 extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'korkab/publish_data_p2/' );
	...
	function show() {
		...
		$data['title'] = 'Publish Data Prelist (P2)';
		$this->template->title( 'Daftar Unpublished' );
		$this->template->content( "korkab/view_publish_data", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
				EXEC dbo.stp_daftar_semua_data_p2 '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',0
			";
			...
			$sql_total = "
				EXEC dbo.stp_daftar_semua_data_p2 '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',1
			";
	...
	function act_publish()
	{
		...
		for ($i=0; $i < count($ids); $i++) {
			...
			$data_log['description'] = 'Publish dari P2 ke P3';
			$data_log['stereotype'] = 'P3';
		...
		$result = [
			...
			'sukses' => $sukses,
			'gagal' => $gagal,
			'pesan' => $sukses . ' data berhasil dipublish. ' . $gagal .' data gagal dipublish',
		];
