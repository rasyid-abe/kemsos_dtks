...
class Qc_ruta extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'korkab/qc_ruta/' );
	...
	function show()	{
		...
		$data['title'] = 'Reject Rumah Tangga (M4 atau M4a)';
		$this->template->title( $data['title'] );
		$this->template->content( "korkab/view_qc_ruta", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			foreach($gen_data->result() as $rows)
			{
				...
				<a class="dropdown-item" href="' . base_url( "monitoring/detail_data/get_form_detail/" . enc( ['proses_id' => $rows->proses_id] ) ) . '" target="_blank">Detail</a>
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_semua_data_m4 WHERE $where 1=1 $where_in";
	...
	function act_reject() {
		...
		for ($i=0; $i < count($ids); $i++) {
			$params_get_audit_trail = [
				...
				'select' => 'audit_trails, stereotype, id_prelist, proses_id'
			];
			...
			$id_prelist = get_data( $params_get_audit_trail )->row('id_prelist');
			...
			$l = $this->db->get_where('master_data', ['proses_id' => $ids[$i]])->row();
	...
	function get_detail_art()
	{
		...
		$query = "
			...
			LEFT JOIN dbo.ref_references ref ON mdd.stereotype= ref.code
