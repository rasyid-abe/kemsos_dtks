...
class All_data extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/all_data/' );
	...
	function show() {
		...
		$data['title'] = 'Daftar Semua Data Prelist (RUTA)';
		$this->template->title( $data['title'] );
		$this->template->content( "view_semua_data", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
			EXEC dbo.stp_daftar_semua_data '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',0
			";
			...
			foreach($gen_data->result() as $rows)
			{
				...
				$detail = '<a href="' . base_url( "monitoring/detail_data/get_form_detail/" . enc( ['proses_id' => $rows->proses_id] ) ) . '" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-search"></i></a>';
			...
			$sql_total = "
			EXEC dbo.stp_daftar_semua_data '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_enum']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',1
			";
	...
	function act_revoke() {
		...
		for ($i=0; $i < count($ids); $i++) {
			...
			// cek-assignment-to-deleted
			$arr_stereotype = ['M1','M2','C2a'];
			if ( in_array( $stereotype, $arr_stereotype ) ) {
				...
				$audit_trails[] = [
					...
					"column_data" => [
						"id_prelist" => $ids[$i],
						"stereotype" => 'P3a',
						"description" => 'Revoke dari '.$stereotype .' menjadi P3a'
					],
				...
				$this->db->where('id_prelist', $ids[$i]);
				...
				$l = $this->db->get_where('master_data', ['id_prelist' => $ids[$i]])->row();
				$data_log['proses_id'] = $l->proses_id;
				$data_log['description'] = 'Revoke dari '.$stereotype.' ke P3a';
				$data_log['stereotype'] = 'P3a';
				$data_log['created_by'] = $this->id['user_id'];
				$data_log['created_on'] =  date('Y-m-d H:i:s');
				$data_log['prelist_id'] =  $ids[$i];
				...
				$update_art = $this->db->update('master_data_detail', $upd_data_art, array('proses_id' => $l->proses_id, 'status_edit' => 1));
		...
		$result = [
			...
			'sukses' => $sukses,
			'gagal' => $gagal,
			'pesan' => $sukses . ' data berhasil direvoke. ' . $gagal .' data gagal direvoke',
		];
