...
class All_data_art extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/all_data_art/' );
	...
	function show()	{
		...
		$data['title'] = 'Daftar Semua Anggota Rumah Tangga (ART)';
		$this->template->title( $data['title'] );
		$this->template->content( "view_semua_data_art", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql = "
			EXEC dbo.stp_daftar_semua_data_art '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_keb']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',0
			";
			...
			foreach($gen_data->result() as $rows)
			{
				...
				<div class="btn-group dropdown mr-1 mb-1">
					...
					<div class="dropdown-menu">
						...
						<a class="dropdown-item" href="' . base_url( "monitoring/detail_data/get_form_detail_art/" . enc( ['id' => $rows->id] ) ) . '" target="_blank">Log</a>
					</div>
				</div>';
				$data[]= array(
					$detail,
			...
			$sql_total = "
			EXEC dbo.stp_daftar_semua_data_art '". $region."', '".$input['s_nkrt']."', '".$input['s_addr']."', '".$input['s_stereo']."', '".$input['s_keb']."', '".$data_ins."', '".$in_notin."', '".$start."', '".$length."',1
			";
