...
class Hasil_pemadanan_art extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/hasil_pemadanan_art/' );
	...
	function show()	{
		...
		$data['qc'] = in_array('qc-c', $is_qc) ? 1 : 0;
		$data['cari'] = $this->form_cari();
		$data['base_url'] = $this->dir;
		$data['title'] = 'Hasil Pemadanan Anggota RUTA (Q2 dan Q2a)';
		$this->template->title( $data['title'] );
		$this->template->content( "pusat/view_hasil_art", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			$input['s_nart'] != '' ? $where .= "nama_art LIKE '%" . $input['s_nart'] . "%' AND " : '';
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_semua_data_q2 WHERE $where 1=1 $where_in";
