...
class Hasil_pemadanan_ruta extends Backend_Controller {

	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monitoring/hasil_pemadanan_ruta/' );
	...
	function show()	{
		...
		$data['title'] = 'Hasil Pemadanan RUTA (C2 dan C2a)';
		$this->template->title( $data['title'] );
		$this->template->content( "pusat/view_hasil_ruta", $data );
	...
	function get_show_data() {
		...
		if ((!in_array('p-i-c', $is_pic) && !in_array('q-c', $is_pic)) || $input['s_fill'] != 0) {
			...
			$sql_total = "SELECT COUNT(*) as num FROM vw_semua_data_c2 WHERE $where 1=1 $where_in";
