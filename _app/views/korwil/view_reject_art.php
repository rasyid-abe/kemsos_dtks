            ...
            <div class="row mb-1">
                <div class="col-sm-12">
                    <div class="bg-light-info text-white" role="group">
                        ...
                        <button type="button" class="btn btn-sm bg-light-info" id="btn_dt_reject" name="button"><i class="ft ft-trending-down"></i> Reject</button>
...
$(document).ready( function(){
    ...
    function response_reject_art() {
        if ('<?=$this->session->flashdata('tab')?>' == 'reject_art') {
            if ('<?=$this->session->flashdata('status')?>' == '1') {
                toastr.info('ART berhasi direject.', '<i class="ft ft-check-square"></i> Success!');
            } else {
                toastr.error('ART gagal direject.', '<i class="ft ft-alert-triangle"></i> Error!');
            }
        }
    }
    ...
    $('#cek_dukcapil').click(function() {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {cek_dukcapil:cek_dukcapil,
                nik:nik},
            dataType: "json",
            success : function(data) {
                alert(data.pesan);
